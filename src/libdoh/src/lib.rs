...
const BASE64_URL_SAFE_NO_PAD: base64::engine::GeneralPurpose = base64::engine::general_purpose::GeneralPurpose::new(
...
impl DoH {
  ...
  async fn serve_get(&self, req: Request<Body>, subscriber: Option<String>) -> Result<Response<Body>, http::Error> {
  ...
  async fn serve_post(&self, req: Request<Body>, subscriber: Option<String>) -> Result<Response<Body>, http::Error> {
    match Self::parse_content_type(&req) {
  ...
  async fn serve_doh_query(&self, query: Vec<u8>, subscriber: Option<String>) -> Result<Response<Body>, http::Error> {
  ...
  fn query_from_query_string(&self, req: Request<Body>) -> Option<Vec<u8>> {
    ...
    let query = match question_str.and_then(|question_str| BASE64_URL_SAFE_NO_PAD.decode(question_str).ok()) {
      Some(query) => query,
      _ => return None,
    };
    Some(query)
  ...
  async fn serve_odoh_proxy(
    ...
    let target_uri = match relays_host_path {
      None => decode(&format!("https://{}{}", targethost, targetpath))
      ...
      Some(relays_hp) => {
        ...
        let targeturl = format!(
          ...
          relays_hp[0].0, relays_hp[0].1, targethost, targetpath, remained_relays
        );
        debug!("[MODoH] Target URL with intermediate relays: {}", targeturl);
        decode(&targeturl).unwrap_or(std::borrow::Cow::Borrowed("")).to_string()
      }
    };
  ...
  async fn serve_odoh_proxy_post(
    ...
    match Self::parse_content_type(&req) {
      Ok(DoHType::Oblivious) => {
        ...
        self
          .serve_odoh_proxy(encrypted_query, &targethost, &targetpath, relays_host_path, subscriber)
          .await
      }
  ...
  fn acceptable_content_type(headers: &HeaderMap, content_types: &[&'static str]) -> Option<&'static str> {
    ...
    for part in accept.to_str().unwrap_or("").split(',').map(|s| s.trim()) {
      if let Some(found) = part.split(';').next().map(|s| s.trim().to_ascii_lowercase()) {
        if let Some(&content_type) = content_types.iter().find(|&&content_type| content_type == found) {
  ...
  fn parse_content_type(req: &Request<Body>) -> Result<DoHType, Response<Body>> {
    ...
    let content_type = match headers.get(hyper::header::CONTENT_TYPE) {
      None => {
        let acceptable_content_type = Self::acceptable_content_type(headers, &[CT_DOH, CT_ODOH]);
  ...
  async fn proxy(&self, query: Vec<u8>, subscriber: Option<String>) -> Result<DnsResponse, DoHError> {
  ...
  async fn _proxy(&self, mut query: Vec<u8>, subscriber: Option<String>) -> Result<DnsResponse, DoHError> {
    ...
    if globals.requires_dns_message_parsing {
      ...
      let query_keys = utils::RequestKey::try_from(&dns_msg).map_err(|_| DoHError::InvalidData)?;
        ...
        match execution_result.action {
          ...
          _ => {
            ...
            if let Some(r_msg) = execution_result.response_msg {
              packet = utils::encode_dns_message(&r_msg).map_err(|_| DoHError::InvalidData)?;
            } else {
    ...
    if go_upstream {
      ...
      // UDP
      {
        ...
        socket
          ...
          .map_err(DoHError::Io)
          .await?;
        let (len, response_server_address) = socket.recv_from(&mut packet).map_err(DoHError::Io).await?;
      ...
      if dns::is_truncated(&packet) {
        ...
        if self.globals.max_clients >= UDP_TCP_RATIO && clients_count >= self.globals.max_clients / UDP_TCP_RATIO {
        ...
        .map_err(DoHError::Io)?;
        let mut ext_socket = socket.connect(globals.server_address).await.map_err(DoHError::Io)?;
        ...
        ext_socket.read_exact(&mut binlen).await.map_err(DoHError::Io)?;
        ...
        ext_socket.read_exact(&mut packet).await.map_err(DoHError::Io)?;
      }
    }
    ...
    dns::add_edns_padding(&mut packet).map_err(|_| DoHError::TooLarge).ok();
    ...
    I: AsyncRead + AsyncWrite + Send + Unpin + 'static,
  {
    ...
    self.globals.runtime_handle.clone().spawn(async move {
      tokio::time::timeout(
        ...
        server.serve_connection(stream, DoHWithPeerAddr { doh: self, peer_addr }),
      )
      .await
      .ok();
      clients_count.decrement();
  ...
  async fn start_without_tls(self, listener: TcpListener, server: Http<LocalExecutor>) -> Result<(), DoHError> {
  ...
  pub async fn entrypoint(self) -> Result<(), DoHError> {
    ...
    let listener = TcpListener::bind(&listen_address).await.map_err(DoHError::Io)?;
    ...
    #[cfg(feature = "tls")]
    {
      tls_enabled = self.globals.tls_cert_path.is_some() && self.globals.tls_cert_key_path.is_some();
    }
    if tls_enabled {
